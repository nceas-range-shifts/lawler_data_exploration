...
```{r}
# Test out taxize package
library(taxize)
#gnr_resolve() depreciated 
temp <- gna_verifier(c("Aspidoscelis arizonae", "Phalacrocorax urile", "Tamias alpinus", "Zoothera naevia",
                       "Dipodomys ingens"))

# finds the new names for some, but not all - maybe using the taxon_synonyms from IUCN (below) will help
```
...
species_vec <- entry_exit_summary$species
```
...
iucn_id_list <- list()
for(i in 1:58){
  test <- iucn_id(api, gns_vec[i], species_vec[i])
  iucn_id_list[[i]] <- test
}
```
...
Endangered giant kangaroo rate (Dipodomys ingens) moving into MX
```{r}
plot(hist_rast$`Dipodomys ingens`, main = "Dipodomys ingens - historical")
plot(future_rast$`Dipodomys ingens`, main = "Dipodomys ingens - future")
```
Near threatened Oregon slender salamander (Batrachoseps wrighti) moving into Canada (population declining)
```{r}
# Batrachoseps wrighti (NT) move into Canada, population declining 
# Oregon slender salamander
plot(hist_rast$`Batrachoseps wrighti`, main = "Batrachoseps wrighti - historical")
plot(future_rast$`Batrachoseps wrighti`, main = "Batrachoseps wrighti - future")
```
Vulnerable beach vole (Microtus breweri) moving to MX (population stable) 
(a small portion of historical range was already in CA)
```{r}
plot(hist_rast$`Microtus breweri`, main = "Microtus breweri - historical")
plot(future_rast$`Microtus breweri`, main = "Microtus breweri - future")
```
```{r}
# Filter further for those entering/exiting CA and MX
# 11 species moving in or out of mexico
entry_exit_CA_MX_summary_nonLC <- entry_exit_endemic_spp %>% 
  filter(country != "United States") %>% 
  filter(description.x != "Least Concern")

# These species are endemic to the US
CA_MX_summary_nonLC <- entry_exit_CA_MX_summary_nonLC %>% 
  group_by(class_name, description.x, juris, status, description.y) %>% 
  summarize(n_species = n()) %>% 
  rename(red_list_cat = description.x,
         population_trend = description.y)
```

This subsample, all the endemic, non-least concern species are mammals or amphibians
```{r}
print(CA_MX_summary_nonLC)
```
