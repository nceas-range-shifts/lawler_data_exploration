...
temp <- gna_verifier(spp_vec[1:20]) %>% 
...
```{r}
library(taxize)
temp_2 <- gna_verifier(spp_vec[1:50]) %>% 
  separate(currentCanonicalSimple, into = c("current_genus", "current_species"), sep = " ", remove = FALSE)
#spp_currentnames <- temp_2$currentCanonicalSimple
```
...
gns_vec_current <- temp_2$current_genus
species_vec_current <- temp_2$current_species
```
