---
title: "IUCN + spp crossing - PCA clean"
format: 
  html:
...
# MCA plot
ggplot(data = ca_sed_mca_var_df, aes(x = X1, y = X2, label = rownames(ca_sed_mca_var_df))) +
...
ggplot(data = ca_sed_mca_obs_df, aes(x = X1, y = X2)) +
  geom_hline(yintercept = 0, color = "gray") +
  geom_vline(xintercept = 0, color = "gray") +
  geom_point(aes(color = hist_or_future), alpha = 0.5) +
  geom_polygon(data = ca_sed_mca_hull,
               aes(x = X1, y = X2,
                   fill = hist_or_future), alpha = 0.25) +
  geom_text(data = ca_sed_mca_var_df,
            aes(x = X1, y = X2, 
                label = rownames(ca_sed_mca_var_df), color = Variable)) +
