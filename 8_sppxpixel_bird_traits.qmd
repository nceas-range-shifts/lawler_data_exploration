...
```{r}
# How many active trait groups
trait_group <- us_spp_trait %>% 
  ...
  summarize(pixel_count = n(),
            # A lot of these trait groups only have one species - need to take closer look (and consider different groupings)
            species_count = length(unique(spp)))
```
...
```{r}
#Breaking into pieces for the IUCN API processing 
...
# Run the assessment numbers collected from the iucn_id function
# 54 unique assessment IDs (have species repeaeted in historic and future)
iucn_id_data_200 <- assessment_data_many(api, iucn_id_vector_200, wait_time = 0.5)
```
...
```{r}
iucn_xwlk_status <- left_join(lawler_iucn_xwlk, iucn_all_clean)
```
