...
# Create a map of pixels with their SSI
...
# Testing out rasterizing the shapefile grid and then treating the pixels like states
```{r}
pixel_rast <- rasterize(grid_sf, hist_rast, field = 'id')
```
...
# Can see that the map isn't working, but the SSI above still seems correct

```{r}
# Merge in the SSI 
grid_ssi_sf <- inner_join(pxl_ssi, grid_sf)
```

```{r}
ggplot() +
  geom_sf(data = grid_ssi_sf %>% 
            filter(juris == "California"), aes(geometry = geometry, fill = ssi, color = ssi))
```
