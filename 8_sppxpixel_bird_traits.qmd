...
ggplot() +
  ...
  # Show areas 0 species and trait overlap in navy
  geom_sf(data = id_rast_sf_us_ssi_diff %>% 
...
One issue is that some of the Lawler birds didn't match with the Avonet traits. This may be causing some of the zero species or trait overlap.

```{r}
lawler_not_avvonet_birds <- birds_avonet %>% 
  filter(is.na(sequence))
```

Read in crosswalk of lawler to avonet species (created with gemini)
...
lawler_avonet_traits <- inner_join(lawler_avonet_xwlk, 
                                   avonet %>% rename(avonet_spp = species1))
```

# Next steps 
Correct the avonet names at the beginning of the code so that all the bird species are included. Then can more accurately compare the species SSI and the trait SSI and highlight areas that are true zeros for both. 
