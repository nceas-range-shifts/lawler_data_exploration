...
ggplot() +
  ...
  labs(title = "Michigan")
```
## Try with some categorical variables - looks strange, and have to use numbers as categories
...
```{r}
ggplot() +
  geom_point(data = mi_data, aes(x = migration, y = hand_wing_index, color = status)) +
  geom_polygon(data = mi_hull, aes(x = migration, y = hand_wing_index, fill = status), alpha = 0.3) +
  labs(title = "Michigan")
```
